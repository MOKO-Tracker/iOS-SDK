...
- (BOOL)scanForPeripheralsWithServices:(NSArray<CBUUID *> *)services options:(NSDictionary<NSString *,id> *)options {
    if (self.centralManager.state != CBCentralManagerStatePoweredOn || self.managerAction != mk_managerActionDefault) {
...
- (void)disconnect {
    if (self.centralManager.state != CBCentralManagerStatePoweredOn || !self.peripheralManager || !self.peripheralManager.peripheral) {
        return;
    }
    [self.operationQueue cancelAllOperations];
...
- (void)updateCentralManagerState {
    MKCentralManagerState managerState = (self.centralManager.state == CBCentralManagerStatePoweredOn ? MKCentralManagerStateEnable : MKCentralManagerStateUnable);
